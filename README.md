...
2. **Clonez** votre fork :
   ```bash
   git clone https://github.com/VOTRE_LOGIN/Data-Trainer.git
   cd Data-Trainer/BIU/MSSM/etudiants
   ```
3. **Complétez** `td_exercices/TD_Exercices.ipynb` : remplissez toutes les cellules `# À COMPLÉTER`.
4. **Testez en local** avant de pousser :
   ```bash
   cd td_exercices
   python ../../prof/scripts/test_td.py --notebook TD_Exercices.ipynb
   ```
   Vous verrez votre score immédiatement. Corrigez jusqu'à être satisfait.
5. **Committez et poussez** :
   ```bash
   cd ../../..
   git add BIU/MSSM/etudiants/td_exercices/TD_Exercices.ipynb
